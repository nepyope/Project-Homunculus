...
public class HandRotator : MonoBehaviour
{
    ...
    public int thumbM = 4; // The mth value in the array for rotating thumb2
    public int thumbD = 2;
    ...
    public float thumbRotationScale3 = 3f; // Scale the rotation amount for thumb2
    ...
    private Quaternion originalRotationThumb3;
    ...
    private float[] thumbStack3 = new float[stackSize];
    ...
    void Start()
    {
        ...
        if (thumb2 != null)
            originalRotationThumb3 = thumb3.localRotation;
    ...
    void Update()
    {
        if (serialController != null && serialController.sensorValues.Length > Mathf.Max(indexNY, indexNZ, indexM, middleNY, middleNZ, middleM, ringNY, ringNZ, ringM, pinkyNY, pinkyNZ, pinkyM, thumbNY, thumbNZ, thumbM,thumbD))
        {
            RotateFingers();
        }
    }
    ...
    void RotateFingers()
    {
        ...
        if (thumb1 != null && thumb2 != null)
        {
            ...
            thumb2.localRotation = originalRotationThumb2 * Quaternion.Euler(0, 0, AverageStack(thumbStackZ1));
            thumb1.localRotation = originalRotationThumb1 * Quaternion.Euler(0, AverageStack(thumbStackY1), AverageStack(thumbStack2));
        }

        if (thumb3 != null)
        {
            UpdateStack(thumbStack3, serialController.sensorValues[thumbD] * thumbRotationScale3 - thumbOffset3);
            thumb3.localRotation = originalRotationThumb3 * Quaternion.Euler(0, 0, AverageStack(thumbStack3));
        }
